...
const app = express();
app.use(express.json()); // Обязательно для работы платежей
...
if (MONGODB_URI) {
    mongoose.connect(MONGODB_URI).then(() => {
        ...
        // Автоматическая установка вебхука для получения чеков об оплате
        fetch(`https://api.telegram.org/bot${BOT_TOKEN}/setWebhook?url=${APP_URL}/webhook`);
    });
}

// ОБРАБОТЧИК ПЛАТЕЖЕЙ (Убирает бесконечную загрузку)
app.post('/webhook', async (req, res) => {
    const update = req.body;

    // 1. Моментальное подтверждение для Telegram (PreCheckout)
    ...
    // 2. Начисление звезд после успешной оплаты
...
function runGame() {
    ...
    // Комиссия: победитель 100%, с проигрышных 5%
    ...
    setTimeout(async () => {
        ...
        setTimeout(() => { 
            gameState.players = []; gameState.bank = 0; gameState.isSpinning = false; io.emit('sync', gameState); 
        }, 5000);
    }, 11000);
}
